...
# 版本: Final v2.9.4 (SNI 优选 + WARP 分流版)
# 更新日志 (v2.9.4):
# - [新增] 支持配置 Cloudflare WARP 本地代理 (Socks5:40000) 进行出站分流
# - [新增] 自动生成分流路由规则 (国内直连/国外走 WARP)
# - [保留] v2.9.3 的所有功能 (SNI 优选, bc 检查等)
# ==============================================================================
...
# --- 全局常量 ---
readonly SCRIPT_VERSION="Final v2.9.4 (WARP Edition)"
...
is_warp_running() {
    # 检测本地 40000 端口是否被监听
    if ss -tlpn 2>/dev/null | grep -q ":40000 "; then
        return 0
    else
        return 1
    fi
...
prompt_for_warp() {
    echo ""
    read -p "$(echo -e " -> 是否配置流量经由 WARP (本地端口 40000) 出站以隐藏真实 IP? [y/N] (默认: ${cyan}N${none}): ")" use_warp_routing || true
    [[ -z "$use_warp_routing" ]] && use_warp_routing="n"

    if [[ "$use_warp_routing" =~ ^[yY]$ ]]; then
        if is_warp_running; then
            success "检测到本地端口 40000 已开启，WARP 模式将启用。"
        else
            warning "未检测到本地端口 40000 (WARP)。"
            echo -e "${yellow}注意: 脚本将配置 Xray 使用 WARP 出口，但您似乎尚未运行 WARP 客户端。${none}"
            echo -e "请在脚本结束后，手动安装 WARP 并运行: ${cyan}warp-cli set-mode proxy${none} (监听端口40000)"
            read -p "是否继续配置 WARP 出口规则? [y/N]: " confirm_warp || true
...
write_config() {
    ...
    # 基础出站: 直连
    local outbound_direct='{ "tag": "direct", "protocol": "freedom", "settings": { "domainStrategy": "UseIPv4v6" } }'
    
    # WARP 出站: Socks5 指向本地 40000
    local outbound_warp='{ "tag": "warp-proxy", "protocol": "socks", "settings": { "servers": [{ "address": "127.0.0.1", "port": 40000 }] } }'
    
    # 路由规则: 分流逻辑
    # 如果启用 WARP，则 CN 直连，其他走 WARP；否则全部默认直连
    local routing_rules='[]'
    local outbounds_list="[$outbound_direct]"

    if [[ "$use_warp_routing" =~ ^[yY]$ ]]; then
        outbounds_list="[$outbound_direct, $outbound_warp]"
        # 分流规则: 国内域名/IP走 direct, 其余走 warp-proxy
        routing_rules='[
            { "type": "field", "outboundTag": "direct", "domain": ["geosite:cn"] },
            { "type": "field", "outboundTag": "direct", "ip": ["geoip:cn", "geoip:private"] },
            { "type": "field", "outboundTag": "warp-proxy", "network": "tcp,udp" }
        ]'
    fi
    ...
    config_content=$(jq -n \
        ...
        --argjson outbounds "$outbounds_list" \
...
prompt_for_vless_config() {
    ...
    if [[ "$use_auto_sni" =~ ^[yY]$ ]]; then
        # 捕获函数输出的最后一行作为结果，同时 stderr 会显示在屏幕上
