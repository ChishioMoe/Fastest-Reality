...
if [ -f "/root/reality.json" ] && [ -f "/root/sing-box" ]; then
    ...
    echo "1. 重新安装 (下载新内核)  2. 更改配置 (保留内核)  3. 卸载"
    read -p "选择 (1-3): " choice
    case $choice in
        1) systemctl stop sing-box; rm /root/sing-box ;; # 删除旧内核以便后续触发下载
        2) SKIP_DOWNLOAD=true ;;
        3) systemctl stop sing-box; rm /etc/systemd/system/sing-box.service /root/reality.json /root/sing-box /root/public.key.b64; echo "卸载完成"; exit 0 ;;
    esac
fi

# --- 安装/配置流程 ---

# 1. 选择版本 (如果之前选择了跳过，则不执行下载)
if [ "$SKIP_DOWNLOAD" != "true" ]; then
    echo "选择内核版本:"
    echo "1. 稳定版 (Stable)"
    echo "2. 测试版 (Pre-release)"
    [ -f "/root/sing-box" ] && echo "3. 使用当前已有的内核 (不重新下载)"
    read -p "选择 (默认1): " v_choice
    v_choice=${v_choice:-1}

    if [ "$v_choice" -eq 3 ] && [ -f "/root/sing-box" ]; then
        echo "跳过下载，使用现有内核..."
    else
        ...
        echo "正在下载版本: $latest_version_tag..."
